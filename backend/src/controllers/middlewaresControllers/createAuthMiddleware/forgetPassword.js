...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const nexacrm_app_email = settings['nexacrm_app_email'];
  const nexacrm_base_url = settings['nexacrm_base_url'];

  const url = checkAndCorrectURL(nexacrm_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | NexaCRM',
    nexacrm_app_email,
