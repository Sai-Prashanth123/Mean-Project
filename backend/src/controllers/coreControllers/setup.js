...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      nexacrm_app_email: email,
      nexacrm_app_company_email: email,
      nexacrm_app_timezone: timezone,
      nexacrm_app_country: country,
      nexacrm_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully NexaCRM App Setup',
  });
};
